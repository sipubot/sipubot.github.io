...
class StockChart {
    ...
    static renderMainChart(history, period) {
        ...
        // recent/summary: "vol" / month_summary/year_summary: "v"
        const volMax = Math.max(...validData.map(h => parseFloat(h.vol || h.v) || 0)) || 1;
        const volData = validData.map(h => (parseFloat(h.vol || h.v || 0) / volMax) * 100);
...
class StockUI {
    ...
    static changePeriod(period) {
        ...
        const history = STATE.detailData.history[period];
        StockChart.renderChart(history, period);

        // 선택 기간의 최신 레코드로 score_breakdown 업데이트
        if (history?.length > 0) {
